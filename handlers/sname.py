...
def song(client, message):
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    m = message.reply('Please wait... I am searching for your song! 😊️. **Join @Animemusicarchive6**')
    ...
    try:
        ...
        print(str(e))
        return
    m.edit("__Your Song is Downloading...__  😉️ Do join **@Animemusicarchive6**")
    try:
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            info_dict = ydl.extract_info(link, download=False)
            audio_file = ydl.prepare_filename(info_dict)
            ydl.process_info(info_dict)
        rep = '**Your Song Is Successfully Uploaded 😌️! Join @Animemusicarchive6❤️** 🙂️'
